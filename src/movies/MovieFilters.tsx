...
const MovieFilters = memo(({ selectedType, onTypeChange, selectedGenre, onGenreChange }: MovieFiltersProps) => {
  ...
  return (
    <div className="flex flex-col bg-gray-950 border border-yellow-400/20 rounded-2xl overflow-hidden sticky top-4 h-fit shadow-2xl">
      {/* Sidebar header */}
      ...
      <div className="px-3 pt-4 pb-2">
        <p className="text-xs text-gray-500 uppercase tracking-widest font-bold px-2 mb-2">Type</p>
        <div className="flex flex-col gap-1">
          {types.map((type) => (
            ...
            >
              <span>{type.value === '' ? '🌐' : type.value === 'movie' ? '🎥' : '📺'}</span>
              {type.label}
            </button>
          ))}
        </div>
      ...
      <div className="px-3 pb-4">
        <p className="text-xs text-gray-500 uppercase tracking-widest font-bold px-2 mb-2">Genre</p>
        <div className="flex flex-col gap-1">
          {genres.map((genre) => (
            <button
              key={genre.value}
